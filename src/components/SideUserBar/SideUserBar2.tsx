...
export function SideUserBar2(props: SideUserBarProps) {
  const [isSideBarOpen, setisSideBarOpen] = useState<boolean>(true);
  const [isHover, setIsHover] = useState<boolean>(false);
  const isMd = useMediaQuery(`(max-width: 880px)`);
  const showWhiteBoardToggleButton = useMediaQuery(`(max-width: 1500px)`);
  ...
  return (
    <>
      {props.itemSelected !== SideBarItems.WhiteBoard && props.activeTab !== TeachTabs.teach && (
        <Box
          style={{
            position: "fixed",
            right: isMd ? 0 : 20,
      ...
      {(props.activeTab !== TeachTabs.teach || showWhiteBoardToggleButton) && <Box
        style={{
          position: "fixed",
          top: isMd?"87%":"90%",
          right: isMd ? 0 : 20,
